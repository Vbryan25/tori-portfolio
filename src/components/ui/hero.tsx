...
// ShaderBackground — generated by the 21st.dev Shader Builder ("Dithering").
// Adapted from Paper Shaders (https://shaders.paper.design/dithering),
// licensed Apache-2.0: https://github.com/paper-design/shaders
//
// Zero dependencies: one WebGL1 canvas that fills its parent. Drop it behind
// your content with
...
void main() {
  ...
  // coordinates before field transforms, so Zoom/Rotate keep their feel.
...
const UNIFORMS = {
  // #1A1423, #B75D69, #EACDC2, #FFF5EB — low to high. Only the first
  // `colorCount` entries are read; the tail is padding for the fixed-size array.
  colors: [
    [0.102, 0.078, 0.137],
    [0.718, 0.365, 0.412],
    [0.918, 0.804, 0.761],
...
export function ShaderBackground({
  ...
  useEffect(() => {
    ...
    // Arrow functions rather than the upstream `function` declarations: those
    // are hoisted above the `if (!canvas) return` / `if (!gl) return` guards,
    // so TypeScript won't narrow either to non-null inside them.
    const requestRender = () => {
